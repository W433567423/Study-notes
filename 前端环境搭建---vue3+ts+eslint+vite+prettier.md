...
- 安装dotenv依赖
- 安装@types/node `npm install @type/node`
- 安装 good-storage `npm install good-storage`
- 安装eslint相关依赖
	1. 列表项
		```
		npm install eslint eslint-plugin-vue vue-eslint-parser @typescript-2. 
		eslint/parser @typescript-eslint/eslint-plugin -D
		```
	2. `npx eslint --init`
